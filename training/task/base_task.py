...
class BaseTask(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ## early stopping
        self.early_stop_count = hparams["early_stop_count"]
        self.early_stop_loss_value = hparams["early_stop_loss_value"]
        self.early_stop_checked = (
            0  # if early_stop_checked > early_stop_count, stop!! check at on_epoch_end
        )
    ...
    def on_epoch_end(self):
        ...
        # early stopping
        if loss_outputs["total_loss"] < self.early_stop_loss_value:
            self.early_stop_checked += 1
            print(
                f"EARLY stop count checked : {self.early_stop_checked} ... loss: {loss_outputs['total_loss']}"
            )

        if self.early_stop_checked > self.early_stop_count:
            print(f"Early Stopped: {self.early_stop_count} times checked")
            print(f"Total loss: {loss_outputs['total_loss']}")
            exit()
    ...
    def start(cls):
        ...
        trainer = BaseTrainer(
            ...
            # early_stop_loss_value=hparams["early_stop_loss_value"],
        )
